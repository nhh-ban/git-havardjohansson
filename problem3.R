...
cat(raw_file[1:(L-3)], sep = "\n", file = "Variablenames.txt")
...
# Create two separate plots, one for values below 11 and one for values over 11
plot_below_11 <- ggplot(data = subset(galaxies, m_b > -11), aes(x = name, y = m_b)) +
  geom_point() +
  labs(title = "Values Below 11")

plot_above_11 <- ggplot(data = subset(galaxies, m_b <= -11), aes(x = name, y = m_b)) +
  geom_point() +
  labs(title = "Values Above 11")

# Arrange the two plots side by side
grid.arrange(plot_below_11, plot_above_11, ncol = 2) 
